...
class VQVAEActionTokenizer:
    def __init__(
        self,
        vqvae_model,
        tokenizer: PreTrainedTokenizerBase, #预训练语言模型的tokenizer
        bins: int = 256,
        min_action: int = -1,
        max_action: int = 1,
        offset: int = 256,
    ...
    def __call__(self, action: np.ndarray) -> Union[str, List[str]]:
        """Clip & bin actions to *the last `n_bins` tokens* of the vocabulary (e.g., tokenizer.vocab[-256:])."""
        discretized_action = self.vqvae_model.get_code(action)  #代码本索引
        offsets = torch.tensor([0, self.offset * 1, self.offset * 2, self.offset * 3]).to(discretized_action.device)
        discretized_action = discretized_action + offsets#添加偏移量，保证离散化值是唯一的
