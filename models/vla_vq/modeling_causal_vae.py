...
class ActionVQVAEPE(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        # self.quant_conv= nn.Conv2d(latent_channels, vq_embed_dim, 1)
        self.vq_layer = ResidualVQ(
            dim=self.vq_embed_dim,#128
            num_quantizers=discrete_cfg["groups"],#4
            codebook_size=self.vqvae_n_embed,#256
            kmeans_init=True,
            # sync_codebook = False, # important! if not set, loss will be different when the number of gpus are different
        )
    ...
    def apply_action_type_emb(self, action):
        ...
        b, c, t, n = action.shape #(1, 1, 5, 7)
        assert n == 7, n
        action = action + torch.cat([self.xyz_emb, self.euler_emb, self.gripper_emb], dim=-1)
        #xyz_emb:(1, 21, 1, 3) euler_emb:(1, 21, 1, 3) gripper_emb (1, 21, 1, 1)
        return action
    ...
    def preprocess(self, action):
        ...
        if action.ndimension() == 2:
            action = action.unsqueeze(0)#(1, 5, 7) 正常是5
    ...
    @apply_forward_hook
    def encode(self, x: torch.Tensor, return_dict: bool = True) -> VQEncoderOutput:
        x = self.preprocess(x)#time & pe embed
        h = self.encoder(x)
        # h = self.quant_conv(h)
        h = h.reshape(h.shape[0], -1)
        if not return_dict:
