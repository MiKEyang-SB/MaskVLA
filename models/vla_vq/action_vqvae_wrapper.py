...
class ActionVQVAELossWrapper(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if use_action_type_pe and use_time_pe:
            config = ActionVQVAEPE.load_config(f"{model_path}/config_action_type_pe_time_pe.json")
            self.vqvae = ActionVQVAEPE.from_config(config)
        elif use_action_type_pe:
            config = ActionVQVAEPE.load_config(f"{model_path}/config_action_type_pe.json")
            self.vqvae = ActionVQVAEPE.from_config(config)
        elif use_time_pe:
            config = ActionVQVAEPE.load_config(f"{model_path}/config_time_pe.json")
            self.vqvae = ActionVQVAEPE.from_config(config)
        else:
            config = ActionVQVAE.load_config(f"{model_path}/config.json")
            self.vqvae = ActionVQVAE.from_config(config)
    ...
    def load_checkpoint(self, checkpoint_path, **kwargs):
        ...
        LOGGER.info(f"Load vae checkpoint from {checkpoint_path}, load result: {vqvae_ckpt_load_result}")
    ...
    def get_code(self, x):
        with torch.no_grad():
            latents = self.vqvae.encode(x).latents #(1, 128 * time/5)
            B = latents.shape[0]  # B
            state_rep_flat = latents.view(latents.size(0), -1, latents.size(1))#(1, 1, 128 * time/5)
