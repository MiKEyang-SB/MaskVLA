...
class ResidualVQ(Module):
    ...
    def __init__(
        self,
        *,
        dim, #128 = vq_embed_dim
        num_quantizers: int | None = None, #groups = 4
        codebook_size: int | tuple[int, ...], #vqvae_n_embed = 256
        codebook_dim=None,
        ...
        **vq_kwargs,
        ...
        codebook_input_dim = codebook_dim * heads #128*1
        ...
        codebook_sizes = cast_tuple(codebook_size, num_quantizers)#(256,256,256,256)

        num_quantizers = default(num_quantizers, len(codebook_sizes))#4
        assert len(codebook_sizes) == num_quantizers

        self.num_quantizers = num_quantizers#4

        self.codebook_sizes = codebook_sizes#4
        ...
        self.layers = ModuleList(
            [
                VectorQuantize(
                    dim=codebook_dim,#128
                    codebook_size=layer_codebook_size,
                    codebook_dim=codebook_dim,#128
                    accept_image_fmap=accept_image_fmap,#false
                    **vq_kwargs,
                )
        ...
        self.quantize_dropout_cutoff_index = quantize_dropout_cutoff_index #0
        ...
        if implicit_neural_codebook:
            ...
            self.mlps = (None,) * (num_quantizers - 1)#(None,None,None)
    ...
    def forward(
        ...
        x = self.project_in(x)#(1, 1, 128*w/5)
